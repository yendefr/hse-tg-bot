...
from keyboards.default import role_menu
...
async def get_email(message: Message, state: FSMContext):
    ...
    if await db.check_student(email) != 0 and state_data.get('role') == 'Студент':
        ...
        await Auth.next()
    elif await db.check_teacher(email) != 0 and state_data.get('role') == 'Преподаватель':
